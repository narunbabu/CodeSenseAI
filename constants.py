...
# # Prompt for summarizing an individual code file
# CODE_FILE_PROMPT = """Please provide a concise summary of this code file.
...
# Keep the summary brief and focused on functionality.
...
# # Prompt for summarizing a project (when file_path is not provided)
# PROJECT_SUMMARY_PROMPT = """Please provide a concise project summary based on the following context:
...
# Keep the summary brief and focused on high-level insights."""

# # Prompt for generating a concise file summary from a detailed file summary
# FILE_CONCISE_PROMPT = "Provide a concise summary for the following file summary:\n\n{file_summary}"

# # Prompt for generating an overall concise project summary from aggregated file summaries
# AGGREGATED_SUMMARY_PROMPT = "Using the following file summaries:\n\n{aggregated}\n\nProvide a concise overall project summary."
...
# # New constant for concise HTML report template.
# HTML_CONCISE_REPORT_TEMPLATE = """<!DOCTYPE html>
# <html lang="en">
# <head>
#     <meta charset="UTF-8">
#     <meta name="viewport" content="width=device-width, initial-scale=1.0">
#     <title>{project_name} - Concise Code Summary</title>
#     <style>
#         body {{
#             font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
#             line-height: 1.6;
...
#     <script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script>
#     <script>
#     fetch('concise_code_summary.json')
#       .then(response => response.json())
#       .then(data => {{
#           document.getElementById('projectTitle').innerText = data.project_name + " Concise Code Summary";
#           document.getElementById('projectInfo').innerText = "Concise summary generated using Ollama service";
#           document.getElementById('fileCount').innerText = data.file_count || Object.keys(data.files).length;
#           document.getElementById('totalLines').innerText = data.total_lines || "N/A";
#           document.getElementById('projectSummary').innerHTML = marked.parse(data.project_summary || "");
...
#               const summaryElem = document.createElement('div');
#               summaryElem.className = 'summary-box';
#               summaryElem.innerHTML = marked.parse(file.concise_summary || "");
...
#     <meta charset="UTF-8">
#     <meta name="viewport" content="width=device-width, initial-scale=1.0">
#     <title>Code Summary: {project_name}</title>
#     <style>
#         body {{ font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 20px; color: #333; }}
#         h1, h2, h3 {{ color: #2c3e50; }}
#         .container {{ max-width: 1200px; margin: 0 auto; }}
#         .summary-box {{ background-color: #f9f9f9; border-left: 4px solid #3498db; padding: 15px; margin-bottom: 20px; }}
#         .concise-box {{ background-color: #f0f7ff; border-left: 4px solid #2ecc71; padding: 15px; margin-bottom: 20px; }}
#         .file-header {{ display: flex; justify-content: space-between; align-items: center; }}
#         .file-info {{ font-size: 0.9em; color: #7f8c8d; }}
#         .tabs {{ display: flex; margin-bottom: 15px; }}
#         .tab {{ padding: 10px 15px; cursor: pointer; border: 1px solid #ddd; background: #f1f1f1; }}
#         .tab.active {{ background: #fff; border-bottom: 1px solid #fff; }}
#         .tab-content {{ display: none; }}
#         .tab-content.active {{ display: block; }}
#         #search-input {{ width: 100%; padding: 10px; margin-bottom: 20px; font-size: 16px; }}
#         .project-summary {{ background-color: #e8f4fc; padding: 20px; margin-bottom: 30px; border-radius: 5px; }}
#         #file-list {{ list-style: none; padding: 0; }}
#         #file-list li {{ padding: 8px; border-bottom: 1px solid #eee; cursor: pointer; }}
#         #file-list li:hover {{ background-color: #f5f5f5; }}
#         .file-grid {{ display: grid; grid-template-columns: 1fr 3fr; gap: 20px; }}
#         pre {{ background-color: #f8f8f8; padding: 10px; overflow-x: auto; border-radius: 4px; }}
#         .btn {{ padding: 8px 12px; background: #3498db; color: white; border: none; cursor: pointer; border-radius: 4px; }}
...
#                 document.getElementById('project-summary-content').innerHTML = `<p>${data.project_summary}</p>`;
...
#                     li.textContent = path;
#                     li.onclick = () => showFileSummary(data.files[path], path);
...
#                     <h3>${path}</h3>
#                     <div class="file-info">${fileData.lines} lines | ${fileSize} ${sizeUnit}</div>
#                 </div>
...
DEFAULT_EXCLUDES = [
    ...
    "dist", "build", ".vscode", ".idea", ".DS_Store", "*.pyc", "*.pyo", "*.md","yarn.lock",
    "*.pyd", "*.so", "*.dll", "*.exe", "*.egg-info", "*.egg", "package-lock.json", "*.yaml","dump*","extractFilesContent.js"
]

CODE_EXTENSIONS = [".py", ".js", ".jsx", ".ts", ".tsx", ".html", ".css", ".scss",
                   ".sass", ".php", ".rb", ".java", ".go", ".rs", ".c", ".cpp",
                   ".h", ".hpp", ".cs", ".swift", ".kt", ".sql", ".xml", ".sh", ".bash", ".ps1", ".dockerfile", ".vue"]
